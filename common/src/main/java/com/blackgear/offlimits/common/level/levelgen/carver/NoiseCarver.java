package com.blackgear.offlimits.common.level.levelgen.carver;
...
public class NoiseCarver {
    ...
    public boolean carveSphere(
        ...
        WorldGenContext context = extension.context();
        ...
        if (x >= centerX - maxRadius && z >= centerZ - maxRadius && x <= centerX + maxRadius && z <= centerZ + maxRadius) {
            ...
            if (!context.terrain().areAquifersEnabled() && access.callHasWater(chunk, chunkX, chunkZ, startX, endX, startY, endY, startZ, endZ)) {
                return false;
            } else {
